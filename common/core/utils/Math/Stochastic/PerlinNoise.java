package core.utils.Math.Stochastic;
...
public class PerlinNoise
{
	...
	public double cosineNoise(double x, double y, double z)
	{
		// Constraint the point to a unit cube
		int uc_x = (int)Math.floor(x) & 255;
		int uc_y = (int)Math.floor(y) & 255;
		...
		return lerp(w, cerp(v, cerp(u, c1, c2), cerp(u, c3, c4)),
				cerp(v, cerp(u, c5, c6), cerp(u, c7, c8)));
	}

	private double cerp(double t, double a, double b)
	{
		double c = t * Math.PI;
		double f = (1.0d - Math.cos(c)) * 0.5d;

		return a - f * (a - b);
	}

	private float cerp(float t, float a, float b)
	{
		double c = t * Math.PI;
		double f = (1.0d - Math.cos(c)) * 0.5d;
	...
	public double fractal2D(int o, float x, float z, float f, float a, float pf, float pa)
	{
		double sum = 0;
		do
		{
	...
	public double noise_fractal3D(int octaves, double x, double y, double z)
	{   
		...
		{
			sum += amplitude * cosineNoise(x, y, z);
	...
	private void initPerlin1()
	{
		...
		for(i = 0; i < B; i++)
		{
			...
			g1[i] = (float)((Rand.nextInt(B + B + 1)) - B) / B;

			for(j = 0; j < 2; j++)
				g2[i][j] = (float)((Rand.nextInt(B + B + 1)) - B) / B;
			normalize2(g2[i]);

			for(j = 0; j < 3; j++)
				g3[i][j] = (float)((Rand.nextInt(B + B + 1)) - B) / B;
			normalize3(g3[i]);
		}
